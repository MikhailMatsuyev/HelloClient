...
function MobileGroup({ label, icon, children }: MobileGroupProps) {
  return (
    <RouterMenuGroup
      ...
      contentClassName="group/sheet invisible fixed inset-0 z-30 data-[state=open]:visible"
    >
...
function MobileSubLink({ to, label }: { to: string; label: string }) {
  return (
    <RouterMenuItem
      to={to}
      label={label}
      className="block rounded-lg px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 data-[active]:bg-blue-50 data-[active]:font-medium data-[active]:text-blue-600"
    />
  )
}
